...
public class ProfileNumberCheck {
    ...
    public static boolean isValidInsuranceNumber(String insuranceNumber) {
        ...
        if (!insurancePattern.matcher(insuranceNumber).matches()) {
            return false;
        }

        return insuranceNumber(insuranceNumber);
    }
    ...
    public static boolean isValidAccountNumber(String accountNumber, String bic) {
        ...
        if (!accountPattern.matcher(accountNumber).matches()) {
            return false;
        }

        return accountNumber(accountNumber, bic);
    }
    ...
    private static boolean insuranceNumber(String insuranceNumber) {
        ...
        if (insuranceNumber.substring(insuranceNumber.length() - 2).equals("00"))
            return controlNumber == 100 || controlNumber == 101;

        return controlNumber == Integer.parseInt(insuranceNumber.substring(insuranceNumber.length() - 2));
